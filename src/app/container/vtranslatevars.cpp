...
//---------------------------------------------------------------------------------------------------------------------
VTranslateVars::VTranslateVars(const VSettings *settings)
    :measurements(QMap<QString, QmuTranslation>()), guiTexts(QMap<QString, QmuTranslation>()),
      ...
      stDescriptions(QMap<QString, QmuTranslation>()), settings(settings)
{
    InitMeasurements();
    InitVariables();
    InitFunctions();
 ...
 */
QString VTranslateVars::FormulaFromUser(const QString &formula) const
{
    if (formula.isEmpty())
    {
        return formula;
    ...
    if (loc != QLocale(QLocale::C) && settings->GetOsSeparator())
...
QString VTranslateVars::FormulaToUser(const QString &formula) const
{
    ...
    QLocale loc = QLocale::system();// User locale
    if (loc != QLocale::C && settings->GetOsSeparator())
    {// User want use Os separator
        QList<int> nKeys = numbers.keys();// Positions for all numbers in expression
        QList<QString> nValues = numbers.values();
        for (int i = 0; i < nKeys.size(); ++i)
