 ...
 *
 * A sewing pattern consists of increments (TODO: what are increments?) and one 
...
public:
    ...
    * @param parent TODO: what about this? do we ever use this?
    */
    ...
    */
    ...
    /**
     * @brief Select pattern piece based on name. RENAME: ChangeActiveDraw?
     * @param name pattern piece name
     * @param parse parsing mode
     */
    void           ChangeActivDraw(const QString& name, const Document::Documents &parse = Document::FullParse);
    /**
     * @brief Get selected pattern piece name. RENAME: GetNameOfActiveDraw?
    ...
    /**
     * @brief Return a list of tool pointers. Q: What for? Which tool pointers?
     * @return list of tool pointers
     */
    inline QHash<qint64, VDataTool*>* getTools() {return &tools;}
    /**
     * @brief Get a tool from the tool list by id. NOTE: inconsistent capitalization
     * @param id tool id
     * @return tool
     */
    VDataTool* getTool(const qint64 &id);
    /**
     * @brief Return a list of history record vectors. TODO: describe more about history
     * @return list of history record vectors
     */
    inline QVector<VToolRecord> *getHistory() {return &history;}
    /**
     * @brief Returns the cursor. TODO: why is cursor an int? It's not the cursor position obviously, but what is it?
     * @return cursor
     */
    inline qint64  getCursor() const {return cursor;}
    /**
     * @brief Set the cursor. TODO: like above. NOTE: inconsistent capitalization
     * @param value cursor
     */
    void           setCursor(const qint64 &value);
    /**
     * @brief Set current data set. TODO: very unclear what this does, possibly bad naming. NOTE: inconsistent capitalization
     */
    void           setCurrentData();
    /**
    ...
    /**
     * @brief Increment reference parent objects. TODO: unclear what it does, seems to work on some tool given by id RENAME: IncrementReference
     * @param id parent object id
     */
    void           IncrementReferens(qint64 id) const;
    /**
     * @brief Decrement reference parent objects. TODO: see above RENAME: like above
     * @param id parent object id
     */
    ...
    /**
     * @brief Return long long value of attribute. TODO: what is a long long value? TODO: what is an attribute? RENAME: GetParameterLongLong
    ...
    /**
     * @brief Return string value of attribute. RENAME: see above
     * @param domElement tag in xml tree
     * @param name attribute name
    ...
    /**
     * @brief Return double value of attribute.
     * @param domElement tag in xml tree
     * @param name attribute name
     * @return double value
     */
...
signals:
    ...
    /**
     * @brief Update tool data from file. TODO: has tool data update been done before emitting or must that be done after the signal is emitted?
     */
...
private:
    ...
    /**
     * @brief Set selected pattern piece. RENAME: SetActiveDraw
     * @param name pattern piece name
     */
    void           SetActivDraw(const QString& name);
    /**
     * @brief Finds an element in the current pattern piece by name. RENAME: like above
     * @param name name tag
    ...
    /**
     * @brief Recursively collects all id attribute in file. Throws exception if a duplicate is found. (TODO: which exception?)
     * @param node tag in XML tree
